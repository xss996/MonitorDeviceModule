...
namespace Peiport_commandManegerSystem
{
    public class Server
    {
        ...
        public Server(IPAddress ip, int ServerPort0)//创建类
        {
            ipaIP = ip;
            this.iServerPort = ServerPort0;
            g_lstConnectClientTab = new List<ListenClient>();
        ...
        public List<ListenClient> GetClientList()
        {
            List<ListenClient> lstClient = new List<ListenClient>();
            return g_lstConnectClientTab;
        }
        ...
        public int SendOneClientMsgByIndex(int intLstIndex, byte[] btCmdBuf)//发送给一个客户端信息,供外部使用
        {
            ...
            {
                intre = g_lstConnectClientTab[intLstIndex].sendMegToCilent(btCmdBuf);
            }
            return intre;
        }
